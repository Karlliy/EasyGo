...
function NewsList() {
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        try {
            ...
            CDbShell::Add_S($_COOKIE);
            CDbShell::Connect();
            if('BannersList' == $_POST['fun']){
                CDbShell::query("SELECT RowId, Id, BannerInfo, BannerUrl, BannerDate FROM banner ORDER BY Id ASC");
                if (CDbShell::num_rows() > 0) {
                    while ($Row = CDbShell::fetch_array()) {
                        $Layout .=
                            <<<EOF
                            <li class="cm_slide">
                                <img src="images/cm/{$Row["BannerUrl"]}">
                            </li>
EOF;
                    }
                    CDbShell::DB_close();
            ...
            if('NewsList' == $_POST['fun']){
                CDbShell::query("SELECT `Row`, Open_or_Close, ReleaseTime, Personnel, Detail FROM `news` WHERE Open_or_Close = 1 ORDER BY ReleaseTime desc");
                if (CDbShell::num_rows() > 0) {
                    while ($Row = CDbShell::fetch_array()) {
                        $Layout .=
                            <<<EOF
...
EOF;
            ...
            if('NewsListAll' == $_POST['fun']){
                CDbShell::query("SELECT `Row`, Open_or_Close, LEFT(ReleaseTime,10) as ReleaseTime, Personnel, Detail, CONCAT(LEFT(Detail,25),'...') as DetailList FROM `news` WHERE Open_or_Close = 1 ORDER BY ReleaseTime desc");
                if (CDbShell::num_rows() > 0) {
                    while ($Row = CDbShell::fetch_array()) {
                        $Layout .=
                            <<<EOF

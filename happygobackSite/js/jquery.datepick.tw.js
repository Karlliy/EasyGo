   ...
   (function($) { // Hide the namespace
    ...
    function Datepick() {
        ...
        this._defaults = { // Global defaults for all the date picker instances
            ...
            yearRange: 'c-90:c+0', // Range of years to display in drop-down (年份區間c-90，就是今年往前80年；c+0 就是未來往後0年)
    ...
    $.extend(Datepick.prototype, {
        ...
        _newInst: function(target, inline) {
            ...
            return {
                ...
                siblings: $([])
            }; // Created siblings (trigger/append)
        },
        ...
        _connectDatepick: function(target, inst) {
            ...
            if (showOn == 'button' || showOn == 'both') { // Pop-up date picker when button clicked
                ...
                var trigger = $(this._get(inst, 'buttonImageOnly') ?
                    $('<img/>').addClass(this._triggerClass[useTR]).attr({
                        src: buttonImage,
                        alt: buttonText,
                        title: buttonText
                    }) :
                    $('<button type="button"></button>').addClass(this._triggerClass[useTR]).html(buttonImage == '' ? buttonText : $('<img/>').attr({
                        src: buttonImage,
                        alt: buttonText,
                        title: buttonText
                    })));
        ...
        _doKeyDown: function(event) {
            ...
            if ($.datepick._datepickerShowing)
                switch (event.keyCode) {
                    ...
                    case 13:
                        var sel = $('td.' + $.datepick._dayOverClass[useTR], inst.dpDiv);
                        if (sel.length == 0)
        ...
        _clearDate: function(id) {
            ...
            if (this._get(inst, 'mandatory'))
                return false;
            inst.stayOpen = false;
            inst.dates = (this._get(inst, 'showDefault') ? [this._getDefaultDate(inst)] : []);
        ...
        _setDate: function(inst, date, endDate) {
            ...
            inst.dates = (date.length == 0 ? [] : [this._restrictMinMax(inst, this._determineDate(inst, date[0], new Date()))]);
        ...
        _generateMonthYearHeader: function(inst, drawMonth, drawYear, minDate, maxDate,
            ...
            if (secondary || !changeYear)
                ...
                var formatYear = $.datepick.formatYear ? $.datepick.formatYear : function(v) {
                    return v;
                }
...
(function($) {
    ...
    $.extend($.datepick, {
        formatDate: function(format, date, settings) {
            ...
            //最後在input裡面顯示的日期時間：西元寫法
            return y + '-' + fm(m) + '-' + fm(d);
            //最後在input裡面顯示的日期時間：國曆寫法

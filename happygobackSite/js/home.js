...
$(function() { 
    $.post("Home/Dashboard", data =>{
        ...
        $('span[name="NewSumPrice"]').html(obj[6]);
    });
    ...
    if (location.href.substring(location.href.lastIndexOf('/') + 1) == "Permission"){
        // dttable = $('#PermissionTable').dataTable();
        // dttable.fnClearTable(); //清空一下table
        // dttable.fnDestroy();
        $.post("Home/Permission", data =>{
            ...
            $("input[id='PermissionEdit']").on('click',  function(Event) {
                let PermissionRowId = $(this).data('value');
                sessionStorage.setItem('session_PermissionRowId', PermissionRowId);
                console.log(PermissionRowId);
                
                var mapForm = document.createElement("form");
                mapForm.method = "POST";
                mapForm.action = "Home/Permission_edit";

                // Create an input
                var mapInput = document.createElement("input");
                mapInput.type = "hidden";
                mapInput.name = "PermissionRowId";
                mapInput.value = PermissionRowId;
            ...
            $("input[id='PermissionDel']").on('click',  function(Event) {
                Event.preventDefault();
                document.getElementById('popup_del').style.display='block';
                let PermissionRowId = $(this).data('value');
                sessionStorage.setItem('session_PermissionRowId', PermissionRowId);
                console.log(PermissionRowId);
                $("input[id='PermissionDelCHK']").on('click',  function(Event) {
                    $.post("Home/Permission_add", {fun:"Permission_del",PermissionRowId:sessionStorage.getItem('session_PermissionRowId')}, data =>{
                        console.log(data);
                        eval(data);
                    });
                });
    ...
    $("input[id='AllCheck']").on('click',  function(Event) { // 全選
        $("input[name='Permission[]']").attr("checked",true); 
    });

    $("input[id='AllClear']").on('click',  function(Event) { //全刪
    ...
    if (location.href.substring(location.href.lastIndexOf('/') + 1) == "Permission_add"){
        ...
        $("input[id='add_reset']").on('click',  function(Event) { //全刪
            $("input[id='AdminAccount']").val(""); 
            $("input[id='AdminPW']").val(""); 
            $("input[id='NickName']").val(""); 
            $("select[id='MemberLevel']").val(""); 
        });
    }

    if (location.href.substring(location.href.lastIndexOf('/') + 1) == "Permission_edit"){
        $.post("Home/Permission_edit", {fun:"PermissionEditList",PermissionRowId:sessionStorage.getItem('session_PermissionRowId')}, data =>{
            console.log(data);
            let obj = JSON.parse(data);
            console.log(obj.length);
            $("input[name='AdminAccount']").val(obj[1]);
            $("input[name='NickName']").val(obj[3]);
            $('select[name="MemberLevel"]').val(obj[0]);
            let i = 4;
            while(i < obj.length){
                $("input:checkbox[value='"+obj[i]+"']").attr("checked", true);
                i++;
            }
            $("input[id='edit_confirm']").on('click',  function(Event) {
                ...
                console.log($("input[id='NickName']").val());
                console.log($("select[id='MemberLevel']").val());
                $.post("Home/Permission_edit", {
                    fun:"PermissionEdit",
                    PermissionRowId:sessionStorage.getItem('session_PermissionRowId'),
                    AdminAccount:$("input[id='AdminAccount']").val(),
                    Permission:Permission,
                    NickName:$("input[id='NickName']").val(),
                    MemberLevel:$("select[id='MemberLevel']").val()
        ...
        $("input[id='edit_reset']").on('click',  function(Event) {
            $("input[id='NickName']").val(""); 
            $("select[id='MemberLevel']").val(""); 
        });
    }

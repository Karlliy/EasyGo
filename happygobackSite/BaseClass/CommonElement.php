...
class CommonElement {
	...
	static function CountHoliday($CheckDate, $DelayDay) {
    	...
    	if (CDbShell::num_rows($Result3) == 0) {
    		...
    		else return self::CountHoliday($CheckDate, $DelayDay);
    		
    	}else {
    		return self::CountHoliday($CheckDate, $DelayDay);
    	}
	}
	...
	static function CopyImg($Serial, $Files, $Path)
    {
        ...
        self::GetRandom(10, $_VendorKey);
        //$dir = $this->GetAdmDataPath() . "$path";
        //@mkdir($dir, 0777); 
        $FileName = $_VendorKey. ".".preg_replace('/^.*\.([^.]+)$/D', '$1', $Files['name']);
		// echo $FileName;
        // exit;
	...
	static function GetRandom($length, &$randoma ) {
	...
	static function Run() {
		...
		//echo $_GET["a"];
		//print_r($_SERVER);
		//preg_match('/(\/(.*?)\/(.*?)\?(.*?)|\/(.*?)\/(.*?))$/', $_SERVER["UNENCODED_URL"], $_Searched);
		//echo $_SERVER["UNENCODED_URL"];
		preg_match('/(\/([\w]+))$|(\/([\w]+)\?([\w]+))$/', $_SERVER["UNENCODED_URL"], $_Searched);
		//var_dump($_Searched);
		if (array_search("Game", $_Searched ) != false || $_SERVER["UNENCODED_URL"] == "/Game") {
			Operate::Hall();
			//echo $_Searched[1]."|Game";
			exit;
		}
		if (array_search("Notify", $_Searched ) != false) {
			Operate::DealNotify();
			//echo $_Searched[1]."|Game";
			exit;
		}
